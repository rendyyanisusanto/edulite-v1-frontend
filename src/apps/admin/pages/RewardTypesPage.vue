<template>
  <div class="reward-types-page">
    <!-- Page Header -->
    <div class="page-header">
      <div>
        <h4 class="mb-1">Jenis Reward</h4>
        <p class="text-muted mb-0">Kelola jenis reward siswa</p>
      </div>
      <button class="btn btn-success" @click="showCreateModal">
        <i class="bi bi-plus-circle me-2"></i>Tambah Jenis Reward
      </button>
    </div>
    ...
    <div class="card mb-4">
      <div class="card-body">
        <div class="row g-3">
          <div class="col-md-4">
            <label class="form-label">Tingkat Reward</label>
            <select v-model="filters.level_id" class="form-select" @change="loadRewardTypes">
          ...
          <div class="col-md-4">
            ...
            <select v-model="filters.limit" class="form-select" @change="loadRewardTypes">
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table class="table table-hover">
            <thead>
              <tr>
                <th width="5%">#</th>
                <th width="30%">Nama Jenis Reward</th>
            ...
            <tbody>
              <tr v-if="rewardTypes.length === 0">
                <td colspan="6" class="text-center py-4">
                  <i class="bi bi-inbox fs-1 d-block mb-2 text-muted"></i>
                  <p class="mb-0">Belum ada data jenis reward</p>
                </td>
              </tr>
              <tr v-for="(type, index) in rewardTypes" :key="type.id">
                <td>{{ (pagination.page - 1) * pagination.limit + index + 1 }}</td>
                <td>
                  <strong>{{ type.name }}</strong>
                </td>
                <td>
                  <span class="badge" :class="getLevelBadgeClass(type.rewardLevel?.name)">
                    {{ type.rewardLevel?.name || '-' }}
                  </span>
                </td>
                <td>
        ...
        <div v-if="pagination.totalPages > 1" class="d-flex justify-content-between align-items-center mt-3">
          <div class="text-muted">
            Menampilkan {{ rewardTypes.length }} dari {{ pagination.total }} data
          </div>
          <nav>
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">
              {{ isEditMode ? 'Edit Jenis Reward' : 'Tambah Jenis Reward' }}
            </h5>
          ...
          <div class="modal-body">
            <form @submit.prevent="handleSubmit">
              <div class="mb-3">
                <label class="form-label">Tingkat Reward <span class="text-success">*</span></label>
              ...
              <div class="mb-3">
                <label class="form-label">Nama Jenis Reward <span class="text-success">*</span></label>
                <input 
                  v-model="form.name" 
                  type="text" 
                  class="form-control" 
                  placeholder="Misal: Apresiasi Belajar, Membantu Kebersihan Sekolah"
                  required
                />
              </div>
              <div class="mb-3">
                <label class="form-label">Poin <span class="text-success">*</span></label>
                <input 
                  v-model.number="form.point" 
                  type="number" 
                  class="form-control" 
                  placeholder="Jumlah poin reward"
                  required
                  min="1"
                />
              </div>
              <div class="mb-3">
                ...
                <textarea 
                  ...
                  class="form-control" 
                  rows="3"
                  placeholder="Deskripsi jenis reward"
                ></textarea>
              </div>
            </form>
          </div>
...
const loadRewardTypes = async () => {
  ...
  try {
    ...
    const response = await rewardService.getAllTypes(params)
    rewardTypes.value = response.data || []
    pagination.value = response.pagination || pagination.value
  } catch (error) {
    console.error('Error loading reward types:', error)
    alert('Gagal memuat data jenis reward')
  } finally {
    loading.value = false
  }
}
...
const handleSubmit = async () => {
  ...
  try {
    if (isEditMode.value) {
      await rewardService.updateType(editingId.value, form.value)
      alert('Jenis reward berhasil diupdate')
    } else {
      await rewardService.createType(form.value)
      alert('Jenis reward berhasil ditambahkan')
    }
    modalInstance.hide()
    loadRewardTypes()
  } catch (error) {
...
const confirmDelete = async (type) => {
  if (confirm(`Data "${type.name}" akan dihapus permanen. Lanjutkan?`)) {
    try {
      await rewardService.deleteType(type.id)
      alert('Jenis reward berhasil dihapus')
      loadRewardTypes()
    } catch (error) {
