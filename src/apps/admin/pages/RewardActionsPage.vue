<template>
  <div class="reward-actions-page">
    <!-- Page Header -->
    <div class="page-header">
      <div>
        <h4 class="mb-1">Tindakan / Sanksi Reward</h4>
        <p class="text-muted mb-0">Kelola tindakan dan sanksi untuk setiap tingkat reward</p>
      </div>
      ...
      <div class="card-body">
        <div class="row g-3">
          <div class="col-md-4">
            ...
            <select v-model="filters.level_id" class="form-select" @change="loadRewardActions">
          ...
          <div class="col-md-4">
            ...
            <select v-model="filters.limit" class="form-select" @change="loadRewardActions">
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table class="table table-hover">
            ...
            <tbody>
              <tr v-if="rewardActions.length === 0">
                <td colspan="5" class="text-center py-4">
                  <i class="bi bi-inbox fs-1 d-block mb-2 text-muted"></i>
                  <p class="mb-0">Belum ada data tindakan/sanksi</p>
                </td>
              </tr>
              <tr v-for="(action, index) in rewardActions" :key="action.id">
                <td>{{ (pagination.page - 1) * pagination.limit + index + 1 }}</td>
                <td>
                  <div class="d-flex align-items-center">
                    <i class="bi bi-hammer-fill me-2 text-success"></i>
        ...
        <div v-if="pagination.totalPages > 1" class="d-flex justify-content-between align-items-center mt-3">
          <div class="text-muted">
            Menampilkan {{ rewardActions.length }} dari {{ pagination.total }} data
          </div>
          <nav>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form @submit.prevent="handleSubmit">
              ...
              <div class="mb-3">
                <label class="form-label">Nama Tindakan / Sanksi <span class="text-success">*</span></label>
                <input 
...
const loadRewardActions = async () => {
  ...
  try {
    ...
    const response = await rewardService.getAllActions(params)
    rewardActions.value = response.data || []
    pagination.value = response.pagination || pagination.value
  } catch (error) {
    console.error('Error loading reward actions:', error)
...
const handleSubmit = async () => {
  ...
  try {
    if (isEditMode.value) {
      await rewardService.updateAction(editingId.value, form.value)
      alert('Tindakan/sanksi berhasil diupdate')
    } else {
      await rewardService.createAction(form.value)
      alert('Tindakan/sanksi berhasil ditambahkan')
    }
    modalInstance.hide()
    loadRewardActions()
  } catch (error) {
    console.error('Error saving reward action:', error)
...
const confirmDelete = async (action) => {
  if (confirm(`Data "${action.action_name}" akan dihapus permanen. Lanjutkan?`)) {
    try {
      await rewardService.deleteAction(action.id)
